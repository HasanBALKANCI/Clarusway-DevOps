...
```Dockerfile
FROM ubuntu
RUN apt-get update -y # her komut icin bir layer olusturur.
RUN apt-get install python3 -y # ikinci layer
RUN apt-get install python3-pip -y # ucuncu layer
...
```bash
docker build -t "clarusway/flask-app:1.0" . # -t tag ver, . Dockerfile icin ihtiyac duydugum herhangibir dosya varsa bu dzizinde ara.. eger baska yerde doya varsa path vermek lazim.
docker image ls
```
